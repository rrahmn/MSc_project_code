...
#calibration funcs
def calibrate_camera(images, square_size, rows, columns):
    rows = int(rows.item())
    ...
    for idx, img_tensor in enumerate(images):
        ...
        if ret == True:
            ...
            img_with_corners = cv2.drawChessboardCorners(img_np.copy(), (rows-1, columns-1), corners2, ret)
            ...
            imgs_with_corners_list.append(img_np)
...
def calibrate_real_camera(images, square_size, rows, columns):
    objp = np.zeros(((rows-1)*(columns-1),3), np.float32)
    objp[:,:2] = (np.mgrid[1:rows,1:columns]*square_size).T.reshape(-1,2)
    objpoints = [] # 3D points in real world space
    imgpoints = [] # 2D points in image plane
    successful_indices = []  # This will store the indices of images where corners were detected
    ...
    for idx, image in enumerate(images):
        ...
        if ret == True:
            ...
            # # # Draw and display the corners on the image
            img_with_corners = cv2.drawChessboardCorners(bgr_img.copy(), (rows-1, columns-1), corners2, ret)
            imgs_with_corners_list.append(img_with_corners)
            # Using OpenCV to display the image
            # cv2.imshow('Detected Corners', img_with_corners)
            # cv2.waitKey(0)  # waits until a key is pressed
            ...
            imgs_with_corners_list.append(bgr_img)
    if len(objpoints) == 0:
        return None
    else:
...
def main():
    ...
    subset_length = int(len(test_dataset))
    ...
    with torch.no_grad():
        for camera, test_data in tqdm(enumerate(test_loader, 0), desc="Processing", total=len(test_loader)):
            ...
            for images_used in [1, 5, 10, 15]:
                images = images_with_backgrounds_list[:images_used]

                calibration_output = calibrate_camera(images, square_size, rows, columns)
        ...
        for jitter_index, jitter_params in enumerate(color_jitter_params_grid):
            for blur_index, blur_params in enumerate(gaussian_blur_params_grid):
                ...
                for img_idx, img in tqdm(enumerate(real_images), desc="Processing Real Images"):
                    ...
                    transformed_img = (torch.tensor(to_tensor_transform(blur_transform(jitter_transform(img)))))
                    transformed_images_list.append(transformed_img)



                    
                calibration_output = calibrate_camera(transformed_images_list, torch.tensor(2.5), torch.tensor(10), torch.tensor(7))
